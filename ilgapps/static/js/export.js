...
$(".off-out").on("click", function () {
    ...
    $("#export_word").attr('rol', o_id);
    var src_str = "/offer/offerExportShow/?id=" + o_id;
    $("#export_html_1").attr('src', src_str);
    $("#export_html_3").attr('src', src_str + "&lang=tc");
    $.ajax({
        "dataType": "json",
        "type": "post",
        "url": "/offer/api/getCardAvil/",
        "data": {'offer_id': o_id},
...
$('#export_word').on('click', function (e) {
    var params = {};
    var tmp = new Array();
    tmp.push($(this).attr('rol'));
    params['id'] = JSON.stringify(tmp);
    //params['type'] = 'jpg';
    //if ($('.daoCon-title-ul .daoCon-on').attr('name') == 'daoCon2016') {
    //    params['version'] = "2016";
    //}
    ...
    //    params['lang'] = 'tc';
    //}
    $('#export_word').html('生成中..');
    $('#export_word').attr('disabled', 'disabled');
    $.ajax({
        "dataType": "json",
        "type": "post",
        ...
        "success": function (data) {
            alert('批量权益导出完成！');
            $('#export_word').html('导出Word');
            $('#export_word').removeAttr('disabled');
...
$('#formExportConfirm').on('click', function (e) {
    ...
    $.ajax({
        "dataType": "json",
        "type": "post",
        "url": "/offer/api/exportOfferBatch/",
        "data": params,
        "success": function (data) {
            if (data.ret == 0) {
